...
def build_prompt(sections: Dict[str, str], hashtags: str) -> Dict[str, str]:
    ...
    user_prompt = (
        ...
        "- ALWAYS includes the provided links at the end with short descriptors:\n"
        "  • If youtube_link exists: 'Watch: [youtube_link]'\n"
        "  • If castopod_link exists: 'Listen: [castopod_link]'\n"
        "  • If blog_link exists: 'Read more: [blog_link]'\n"
        "  • If any link is empty or placeholder text, use '[LINK_PLACEHOLDER]' instead\n"
        "- Ends with the supplied hashtags (ensure they are present exactly once).\n"
        "- Sounds natural and human-written, not overly polished or AI-generated.\n"
        "- Uses a conversational tone without excessive emojis or corporate jargon.\n\n"
        "Return ONLY the final post text with no explanations or additional commentary."
    )

    return {
        "system": "You craft concise, engaging LinkedIn posts for podcast episodes. Never include introductory phrases like 'Here's a LinkedIn post' or 'This post captures' or any meta-commentary about the post itself. Start directly with the hook.",
        "user": user_prompt + "\n\n" + "\n\n".join(content_parts),
    }
